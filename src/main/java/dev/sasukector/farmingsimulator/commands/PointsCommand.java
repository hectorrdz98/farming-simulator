    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player player && player.isOp()) {
            if (args.length > 0) {
                ...
                Player searchedPlayer = Bukkit.getPlayer(option);
                if (searchedPlayer != null) {
                    ...
                    ServerUtilities.sendServerMessage(player, ServerUtilities.getMiniMessage()
                            .parse("<bold><color:#0091AD>" + searchedPlayer.getName() +
                                    "</color></bold> lleva <bold><color:#B7094C>" +
                                    PointsController.getInstance().getPlayerPoint(searchedPlayer) +
                                    "</color></bold> puntos"));
                } else if (validOptions(player).contains(option)) {
                    switch (option) {
                        case "total" -> {
                            player.playSound(player.getLocation(), "minecraft:block.note_block.bell", 1, 1);
