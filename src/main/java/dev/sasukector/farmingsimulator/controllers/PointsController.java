...
public class PointsController {
    ...
    public void addPointsToPlayer(Player player, int points) {
        ...
        if (newPoints >= 100 && newPoints < 1000) {
            TeamsController.getInstance().getOneLegTeam().addEntry(player.getName());
        } else if (newPoints >= 1000 && newPoints < 5000) {
        ...
        } else if (newPoints >= 10000 && newPoints < 25000) {
            TeamsController.getInstance().getSpeedTeam().addEntry(player.getName());
        } else if (newPoints >= 25000 && newPoints < 50000) {
            TeamsController.getInstance().getAddictTeam().addEntry(player.getName());
        } else if (newPoints >= 50000 && newPoints < 100000) {
            TeamsController.getInstance().getAdventureTeam().addEntry(player.getName());
        } else if (newPoints >= 100000 && newPoints < 250000) {
            TeamsController.getInstance().getMissionTeam().addEntry(player.getName());
        } else if (newPoints >= 250000) {
    ...
    private JsonArray getPointsJSONArray() {
        ...
        File configFile = new File(FarmingSimulator.getInstance().getDataFolder(), "points.json");
        if (!configFile.exists()) {
            FarmingSimulator.getInstance().saveResource(configFile.getName(), false);
        }
        try {
    ...
    public void createSaveTask() {
        ...
        }.runTaskTimer(FarmingSimulator.getInstance(), 0L, 20L * 10);
    }
